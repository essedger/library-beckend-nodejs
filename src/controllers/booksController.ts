...
import { generateAccessToken, getUserIdByToken, IMyToken } from '../utils/token';
import User from '../models/User';
import mongoose, { Types, Query, ObjectId } from 'mongoose';

const sendGetBooksResponse = async (res:Response, query:  Query<IBook[], IBook, {}, IBook>, page: number, perPage: number) => {
...
class booksController {
  ...
  async getBooks(req: Request, res: Response) {
    try {
      ...
      const booksQuery = Book.find<IBook>()
      if (searchRegex) {
        await booksQuery.or([
          { name: searchRegex },
          { author: searchRegex },
  ...
  async getBookById(req: Request, res: Response) {
    try {
      ...
      const book = await Book.findById(param?.bookId).populate('owner', 'username')
